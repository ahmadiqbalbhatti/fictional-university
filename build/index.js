...
/* harmony import */ var _modules_LiveSearch__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/LiveSearch */ "./src/modules/LiveSearch.js");
...
const liveSearch = new _modules_LiveSearch__WEBPACK_IMPORTED_MODULE_4__["default"]();
...
class LiveSearch {
  ...
  showResults(results) {
    ...
    return `
        ${results.length ? '<ul class="link-list min-list">' : `<p>No Information Matches the search Query!</p>`}
                    ${results.map(result => `<li><a href="/${result.permalink}">
                    ${result.title}</a> ${result.postType === 'post' ? `Post published by ${result.authorName}` : ''} </li>`).join('')}
                    ${results.length ? '</ul>' : ``}
        `;
  }
  getResults() {
...
<!--                    Professors -->
                    <h2 class="search-overlay__section-title">Professors</h2>
                        ${results.professors.length ? '<ul class="professor-cards">' : '<p>No' + ' professors match the search result!</p>'}
...
class MyNotes {
  constructor() {
    this.notes = jquery__WEBPACK_IMPORTED_MODULE_0___default()("#my-notes");
    this.events();
  }
  events() {
  ...
  createNote(e) {
    let noteTitle = jquery__WEBPACK_IMPORTED_MODULE_0___default()(".new-note-title");
    let noteBody = jquery__WEBPACK_IMPORTED_MODULE_0___default()(".new-note-body");
    let ourUpdatedPost = {
      'title': noteTitle.val(),
